...
- [x] Убрать слово `Подписка` с кнопки главного меню (`💳 Баланс`).
- [x] Обновить экран после нажатия `🔮 Задать вопрос` под референс (заголовок + примеры).
- [x] Обновить help-блок: `Поддержка в Телеграм` + `Поддержка в Макс`.
- [x] Добавить поле `support_max_url` в backend billing summary и прокинуть в Flutter-модель.
- [x] Добавить лёгкую анимацию ожидания расклада (пульсирующий `🔮`) для progress-сообщений.
- [x] Прогнать локальные проверки (backend + flutter analyze).
- [x] Выкатить backend на сервер.
- [x] Собрать новую APK.
- [ ] Установить APK на Android (blocked: `adb devices` в этой сессии пустой).
- [x] Закоммитить и запушить изменения.
...
- [x] Поднять версию приложения для новой APK сборки (`pubspec version`).
- [x] Добавить стабильный build-скрипт выпуска APK с именами `apptaro.apk` и `apptaro-v<version>+<build>.apk`.
- [x] Исправить 3-card background fallback: использовать layout background, а при проблеме переходить на runtime fallback `backgrounds/main.png`.
- [x] Обновить layout полного расклада: карты сделать крупнее и увеличить расстояние между ними.
- [x] Подкрутить 2-card continuation image: чуть увеличить размер карт и отступ.
- [x] Прогнать локальные проверки (backend compile/test, flutter analyze).
- [x] Собрать новую APK, попытаться установить на подключённый Android.
- [x] Сохранить smoke-артефакты рендера (2/3 карты) для визуальной валидации.
- [ ] Установка APK на Android через USB: blocked (в текущей shell-сессии `adb devices` показывает пустой список).
- [x] Обновить сервер после backend-изменений.
- [x] Закоммитить и запушить изменения в `main`.
